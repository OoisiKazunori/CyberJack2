...
//レイトレ内で行うライティングパス
void LightingPass(inout float arg_bright, float4 arg_worldPosMap, float4 arg_normalMap, LightData arg_lightData, uint2 arg_launchIndex, DebugOnOffParam arg_debugOnOffParam, RaytracingAccelerationStructure arg_scene, bool arg_isFar)
{
    ...
    if (arg_lightData.m_dirLight.m_isActive && 0.1f < length(arg_normalMap.xyz))    //「ディレクションライトが有効だったら」 かつ 「現在のスクリーン座標の位置に法線が書き込まれていたら(何も書き込まれていないところからは影用のレイを飛ばさないようにするため。)」
    {
        ...
        //レイを撃つ
        bool isDebug = arg_debugOnOffParam.m_debugShadow == 1 && arg_launchIndex.x < arg_debugOnOffParam.m_sliderRate;
